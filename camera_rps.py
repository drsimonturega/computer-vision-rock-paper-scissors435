...
def get_computer_choice(self, weapon_list):
    """
    Selects the computers weapon from the list weapon_list 
...
def get_prediction(self, weapon_list):
    """
    The get_prediction function reads the users choice of weapon 
    from the camera using the keras_model.h5
    ...
    while True:
        ...
        if ((time.time() - start_time) >= 3 or 
            cv2.waitKey(1) & 0xFF == ord('c')):
            prediction = np.ndarray.tolist(prediction)
            self.weapon_of_choice = (self.weapon_list[
            np.argmax(np.array(prediction[0]))])
...
def get_winner(
        ...
        weapon_of_computer, user_wins, 
        computer_wins):
    """
    Function that takes the user choice and computer choice to determin
    the winner using a serise of if-elif-else statements
    
    Keyword arguments:
    weapon_of_choice -- weapon  chosen by user
    weapon_of_computer -- weapon chosen by computer
    user_wins -- counter for number of times user has won a round of RPS
    computer_wins -- counter for number of times 
        computer has won a round of RPS
    """
    # checks choices if user choice is "rock"
    if (self.weapon_of_choice == "rock" and 
        self.weapon_of_computer == "sissors"):
        print(f' Your {self.weapon_of_choice} beats \
        {self.weapon_of_computer}, YOU WIN')
        self.user_wins = self.user_wins + 1
    elif (self.weapon_of_choice == "rock" and 
        self.weapon_of_computer == "paper"):
        print(f' Your {self.weapon_of_choice} is beaten by  \
        {self.weapon_of_computer}, YOU LOSE')
        self.computer_wins = self.computer_wins + 1
    else:
        if (self.weapon_of_choice == "rock" and 
        self.weapon_of_computer == "rock"):
            print(f' Your {self.weapon_of_choice} is the same as \
            {self.weapon_of_computer}, IT\'S A TIE')

    # checks choices if user choice is "sissors"        
    if (self.weapon_of_choice == "sissors" and 
        self.weapon_of_computer == "paper"):
        print(f' Your {self.weapon_of_choice} beats \
        {self.weapon_of_computer}, YOU WIN')
        self.user_wins = self.user_wins + 1
    elif (self.weapon_of_choice == "sissors" and 
        self.weapon_of_computer == "rock"):
        print(f' Your {self.weapon_of_choice} is beaten by  \
        {self.weapon_of_computer}, YOU LOSE')
        self.computer_wins = self.computer_wins + 1
        ...
        if (self.weapon_of_choice == "sissors" and 
            self.weapon_of_computer == "sissor"):
            print(f' Your {self.weapon_of_choice} is the same as  \
            {self.weapon_of_computer}, IT\'S A TIE')
    
    # checks choices if user choice is "paper"
    if (self.weapon_of_choice == "paper" and 
        self.weapon_of_computer == "rock"):
        print(f' Your {self.weapon_of_choice} beats \
        {self.weapon_of_computer}, YOU WIN')
        self.user_wins = self.user_wins + 1
    elif (self.weapon_of_choice == "paper" and 
        self.weapon_of_computer == "sissors"):
        print(f' Your {self.weapon_of_choice} is beaten by  \
        {self.weapon_of_computer}, YOU LOSE')
        self.computer_wins = self.computer_wins + 1
...
def play():
    """
    This is the function that runs the game and 
    determins the winner based upon three rounds of the RPS game
    ...
    while True:
        ...
        get_winner(
            game, weapon_of_choice, 
            weapon_of_computer, computer_wins, 
            user_wins)
